        ...
        while [[ -d ${reporter_untar_spots_dir} ]] && spot_file_list=( $(find ${reporter_untar_spots_dir} -name '*_spots.txt')  ) && [[ ${#spot_file_list[@]} -gt 0 ]]; do
            ...
            if [[ ! -s ${TS_SPOTS_CSV_FILE} ]]; then
                ...
                for split_csv_file in ${split_file_list[@]} ; do
                    ...
                    local ret_code=$?
                    if [[ ${ret_code} -ne 0 ]]; then
                        wd_logger 1 "ERROR: 'python3 ${TS_BATCH_UPLOAD_PYTHON_CMD} ${split_csv_file} --input ${split_csv_file} --sql ${TS_WD_BATCH_INSERT_SPOTS_SQL_FILE} --address localhost --ip_port ${TS_IP_PORT-5432} --database ${TS_WD_DB} --username ${TS_WD_WO_USER} --password ${TS_WD_WO_PASSWORD}  => ${ret_code}\nwhen recording the $( wc -l < ${split_csv_file} ) spots in ${split_csv_file} to the wsprdaemon_spots_s table"
                    else
