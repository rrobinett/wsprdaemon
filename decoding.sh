...
function get_wav_file_list() {
    ...
    while (( ${#return_list[@]} == 0 )); do
        ...
        if [[ ${wait_for_newest_file_to_close} == "yes" ]]; then
            ...
            wait_until_newest_tmp_file_is_closed ${wav_recording_dir} "${wav_file_regex}" ${receiver_name} ${receiver_band}
            wd_logger 2 "Done waiting.  So check for newest *.wav file to be closed"
            wd_logger 2 "Waiting for ${WAIT_FOR_FILE_TO_CLOSE_SECONDS-65} seconds for the newest file ${newest_file_name} to be closed or not written to for ${KIWIRECORDER_WRITE_IS_FINISHED_SECONDS-2} seconds"
            ...
            wd_logger 2 "After waiting for the newest file ${newest_file_name} to be closed, cancel wait_for_newest_file_to_close and check again"
       ...
       for seconds_in_wspr_pkt in ${target_seconds_list[@]} ; do
           ...
           local wav_checked_pkt_sec_list=( $(  find ${wav_recording_dir} -maxdepth 1 -type f -name "${wav_file_regex}.${seconds_in_wspr_pkt}-secs" 2>find.stderr | sort -r  ) ) 
...
function decoding_daemon() {
    ...
    while true; do
        ...
        if [[ ${receiver_name} =~ ^KA9Q && ${KA9Q_METADUMP_STATUS_CHECKING_ENABLED-no} == "yes" ]]; then
            ...
            ka9q_get_current_status_value "channel_rf_gain_value" ${receiver_ip_address} ${receiver_freq_khz} "rf gain"   ### There is also a 'rf gain cal' value in the status file
            rc=$? ; if (( rc )); then
            ...
            wd_logger 2 "ka9q_get_current_status_value() => channel_rf_gain_value='${channel_rf_gain_value}' => ka9q_rf_gain_float='${ka9q_rf_gain_float}'"

            local channel_adc_dbfs_value
            ka9q_get_current_status_value "channel_adc_dbfs_value" ${receiver_ip_address} ${receiver_freq_khz} "IF pwr"
            rc=$? ; if (( rc )); then
            ...
            wd_logger 2 "ka9q_get_current_status_value() => channel_adc_dbfs_value='${channel_adc_dbfs_value}' => ka9q_adc_dbfs_float='${ka9q_adc_dbfs_float}'"

            local channel_n0_value
            ka9q_get_current_status_value "channel_n0_value" ${receiver_ip_address} ${receiver_freq_khz} "N0"
            rc=$? ; if (( rc )); then
            ...
            wd_logger 2 "ka9q_get_current_status_value() => channel_n0_value='${channel_n0_value}' => ka9q_n0_float='${ka9q_n0_float}'"

            local channel_gain_value
            ka9q_get_current_status_value "channel_gain_value" ${receiver_ip_address} ${receiver_freq_khz} "gain"
            rc=$? ; if (( rc )); then
            ...
            ka9q_get_current_status_value "channel_output_level_value" ${receiver_ip_address} ${receiver_freq_khz} "output level"
            rc=$? ; if (( rc )); then
            ...
            wd_logger 2 "ka9q_get_current_status_value() => channel_output_level_value='${channel_output_level_value}' => ka9q_channel_output_float='${ka9q_channel_output_float}'"
            ...
            ka9q_get_current_status_value "ka9q_status_ip" ${receiver_ip_address} ${receiver_freq_khz} "status dest"
            rc=$?
        ...
        if [[ -z "${last_adc_overloads_count}" ]] ; then
            wd_logger 1 "ERROR: last_adc_overloads_count is empty"
            new_sdr_overloads_count=0
        elif (( last_adc_overloads_count == -1 )); then
            ...
            if ! is_uint "${adc_overloads_count}" || ! is_uint "${last_adc_overloads_count}" ; then 
                wd_logger 1 "ERROR: invalid adc_overloads_count='${adc_overloads_count}' or last_adc_overloads_count='${last_adc_overloads_count}'"
                sleep 60
                echo ${force_abort}
            fi
        ...
        for returned_files in ${mode_wav_file_list[@]}; do
            ...
            if [[ ${#receiver_modes_list[@]} -eq 1 && ${receiver_modes_list[0]} == "W0" || " ${receiver_modes_list[*]} " =~ " W${returned_minutes} " ]]; then
                ...
                if (( rc )); then
                    ...
                    if [[ -n "${python_peak_level_dBFS_float}" ]]; then
                        ...
                        wd_logger 2 "Adjust the FFT/C2 and RMS data by ${sdr_noise_level_adjust_float} dB to compensate for the normalization by sox of the decoded wav file"
            ...
            if [[ " ${receiver_modes_list[*]} " =~ " F${returned_minutes} " ]]; then
                ...
                if (( rc )); then
                    ...
                    if [[ ! -s ${decode_dir_path}/decoded.txt ]]; then
                        wd_logger 2 "FST4W found no spots after $(( SECONDS - start_time )) seconds"
                    else
                        ### jt9 found spots
                        ...
                        if [[ -n "${sox_signals_rms_fft_and_overload_info}" ]]; then
                            ...
                            wd_logger 2 "FST4W spot lines can include the noise level information '${sox_signals_rms_fft_and_overload_info}' which was just generated by wsprd"
                        else
                        ...
                        if [[ ! -s ${decode_dir_path}/fst4_decodes.dat ]]; then
                            ...
                            if [[ ! -s ${decode_dir_path}/new_fst4w_decodes.dat ]]; then
                                wd_logger 2 "Found no new FST4W high res spots after filtering out '<...>' spots:\n$(< ${decode_dir_path}/new_fst4w_decodes.dat)"
                            fi
            ...
            if [[ ${processed_wav_files} == "yes" ]]; then 
                wd_logger 2 "Processed files '${wav_files}' concatenated into '${decoder_input_wav_filename}' for packet of length ${returned_seconds} seconds"
            else
