...
declare FFTW_REFERENCE_WISDOM_DIR="${WSPRDAEMON_ROOT_DIR}/wisdom"    ### Wisdom measured once on a quiet machine, one file per FFTW build string, so no site has to measure it under load
...
### Remove the obsolete CHU time-station frequencies (3330/7850/14670 kHz - the CHU stations went off the air in
### 2026) from the WWV-IQ channel of a radiod conf file.  Section-aware in-place edit, with a timestamped backup.
### Idempotent: once the CHU frequencies are gone this makes no further changes to the file.
### This deliberately does NOT add bands.  WD should not change which bands a site receives without being asked.
...
function wd_reconcile_radiod_band_list() {
    ...
    ### Match WHOLE space-separated tokens.  A bare gsub(/7850000/,"") also matches INSIDE a longer
    ### frequency -- it turned "17850000" (17.85 MHz) into "1" -- so split the quoted list and compare
    ### each token exactly.
    gawk '
        /^[[:space:]]*\[/ { section=$0 }
        /^[[:space:]]*freq[[:space:]]*=/ {
            ...
            if (section ~ /WWV-IQ\][[:space:]]*$/ && match($0, /"[^"]*"/)) {
                ...
                for (i = 1; i <= n; i++) {
                    if (tok[i] == "")                                                       continue
                    if (tok[i] == "3330000" || tok[i] == "7850000" || tok[i] == "14670000")  continue
                    out = (out == "" ? tok[i] : out " " tok[i])
                }
                $0 = substr($0, 1, RSTART) out substr($0, RSTART + RLENGTH - 1)
            }
        }
        { print }
    ' "${conf_file}" > "${tmp_file}"
...
### Standard check: remove obsolete CHU time-station entries (the CHU stations went off air in 2026) from
### WSPR_SCHEDULE[] in WD.conf.  Edits WD.conf in place with a timestamped backup, and only applies the change if
### the result still parses (bash -n).  WD re-sources WD.conf every cycle, so the change takes effect without a
### restart.  Idempotent: once the CHU entries are gone this makes no further changes, so WD stops touching the
### user's config file.  This deliberately does NOT add bands -- WD should not change a site's schedule unasked.
### Returns: 0 = already current, 1 = changed, 2 = error.
function wd_reconcile_wspr_schedule() {
    ...
    if cmp -s "${conf_file}" "${tmp_file}"; then
        rm -f "${tmp_file}"
        wd_logger 2 "WSPR_SCHEDULE in ${conf_file} is already current (no CHU entries)"
        return 0
    fi
    ...
    wd_logger 1 "Updating WSPR_SCHEDULE in ${conf_file}: removing the obsolete CHU time-station entries"
...
function wd_fftw_install_reference_wisdom()
{
    ...
    local build_string
    build_string=$( wd_fftw_build_string )
...
### Report the FFTW build string radiod uses on this host, e.g. "fftw-3.3.10-sse2-avx".
### FFTW wisdom is keyed to the CODELET SET, not to the CPU model -- which is why n6gn3's file
### loaded and worked on n6gn5, a different part.  radiod prints the string at every start, and
### that is the only place it is stated exactly as FFTW itself computed it, so read it from the
### journal rather than trying to derive it from the installed library and the CPU flags.
function wd_fftw_build_string()
{
    local unit
    unit=$( systemctl list-units 'radiod@*' --no-legend 2>/dev/null | awk '{print $1}' | head -1 )
    [[ -n ${unit} ]] || return 1
    local line
    line=$( journalctl -u "${unit}" --no-pager 2>/dev/null | grep -m1 "FFTW version:" )
    [[ -n ${line} ]] || return 1
    echo "${line##*FFTW version: }"
    return 0
}
...
function build_ka9q_radio() {
   ...
   local index
    ...
    elif wd_fftw_install_reference_wisdom "${wisdom_spec_list}" "${wisdom_marker_file_path}" ; then
        wd_logger 1 "Installed reference wisdom, so radiod has a measured FFT plan without this host computing one"
...
function ka9q-ft-setup() 
{
    ...
    ### language, so pin the decoder's numeric locale.  It must be LANG: ka9q-radio's jt-decoded.c does
    ###     char const * locale = getenv("LANG"); setlocale(LC_ALL,locale);
    ### so it reads $LANG and hands it straight to setlocale, and LC_ALL alone is IGNORED -- setting only
    ### LC_ALL=C looks right, survives a restart, shows up in /proc/PID/environ, and changes nothing at
    ### all.  Set both, and LANG is the one doing the work.  This affects only number and date
    ### formatting inside this daemon; the operator's own shell is untouched.
    ...
    if [[ ! -f ${ft_decode_locale_dropin} ]] || ! grep -q 'LANG=C' ${ft_decode_locale_dropin} ; then
        ...
        printf '[Service]\nEnvironment="LANG=C"\nEnvironment="LC_ALL=C"\n' | sudo tee ${ft_decode_locale_dropin} > /dev/null
        sudo systemctl daemon-reload
        service_restart_needed="yes"
    fi
...
### 7/3/25 - RR: Wayne is enhancing KA9Q-web for Glenn N6GN to add ITU noise level plots and other reference information to the spectrum display
###              To enable insallation of that enhanced -web, add this one line to wsprdaemon.conf: 
###   N6GN_WEB_MODE="yes"
if [[ ${N6GN_WEB_MODE-no} == "yes" ]] ; then
    KA9Q_RADIO_COMMIT_CHECK="main"
    KA9Q_FT8_COMMIT_CHECK="master"
    PSK_UPLOADER_COMMIT_CHECK="main"
    ONION_COMMIT_CHECK="master"
    KA9Q_WEB_PROJECT_NAME="ka9q-web1"
    KA9Q_WEB_COMMIT_CHECK="main"
    KA9Q_WEB_GIT_URL="https://github.com/wa2n-code/ka9q-web1"
fi
