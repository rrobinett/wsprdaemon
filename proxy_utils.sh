...
function proxy_connection_manager() {
    local proxy_pid=$(proxy_connection_status)
    if [[ ${REVERSE_PROXY-no} == "no" ]] ; then
        if [[ ${proxy_pid} -ne 0 ]]; then
            ...
            wd_logger 0 "Proxy disabled and found no pid file as expected"
        fi
        return
    fi
...
EOF
    ...
    wd_logger 0 "Spawned frpc daemon with pid ${frpc_daemon_pid}"
}
