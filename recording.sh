...
{ 
    ...
    wd_logger 1 "Start reecording pcm wav files from ${receiver_ip}"
    local running_jobs_pid_list=()
    while    running_jobs_pid_list=( $( ps x | grep "${KA9Q_RADIO_WD_RECORD_CMD} -L 60 ${receiver_ip}" | grep -v grep | awk '{ print $1 }' ) ) \
     ...
     wd_logger 1 "Starting '${KA9Q_RADIO_WD_RECORD_CMD} -L 60 ${receiver_ip}' in ${PWD}"
     ${KA9Q_RADIO_WD_RECORD_CMD} -L 60 --jt ${receiver_ip} > wd-record.log 2>&1   ## wd-record prints to stderr, but we want it in wd-record.log
     rc=$?
     if [[ ${rc} -eq 0 ]]; then
         wd_logger 1 "ERROR: Unexpectedly '${KA9Q_RADIO_WD_RECORD_CMD} -L 60 ${receiver_ip}' terminated with no error"
     else
         wd_logger 1 "ERROR: Unexpectedly '${KA9Q_RADIO_WD_RECORD_CMD} -L 60 ${receiver_ip}' => ${rc}"
     fi
     return 1
}
...
function spawn_wav_recording_daemon() {
    ...
    if [[ ${receiver_name} =~ ^KA9Q ]]; then
        wd_logger 1 "Spawning a KA9Q pcmrecorder daemon for receiver '${receiver_name}' in directory ${recording_dir}"
        cd  ${recording_dir}
        WD_LOGFILE=${WAV_RECORDING_DAEMON_LOG_FILE}  ka9q_recording_daemon ${receiver_ip}  &    ### Once instance of pcmrecord outputs all the bands in the stream to a series of wav files
        cd - > /dev/null
    else
